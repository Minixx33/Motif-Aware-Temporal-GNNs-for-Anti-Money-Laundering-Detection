...
def main():
    ...
    parser.add_argument("--graph_suffix", default="", help="Optional suffix for ablation graph folders")
    ...
    graph_folder = os.path.join(root, dyrep_graphs_dir_rel, ds_name + args.graph_suffix)
    ...
    print(f"\nStarting training for up to {epochs} epochs (starting at epoch {start_epoch})...\n")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if should_sync_this_epoch(base_cfg, epoch):
            sync_experiment_to_drive(base_cfg, project_root, results_dir, logs_dir, label=experiment_name)
    ...
    sync_experiment_to_drive(base_cfg, project_root, results_dir, logs_dir, label=f"{experiment_name} (final)")
