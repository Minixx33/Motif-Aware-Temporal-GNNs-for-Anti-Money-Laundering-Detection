...
# Handles large graphs (5M+ edges) efficiently on GPU
# -----------------------------------------------------------
...
@torch.no_grad()
def evaluate_split_minibatch(model, x, edge_index, edge_attr, y_edge, split_idx,
                             batch_size, device, eval_cfg, use_amp=True):
    """Evaluate with mini-batches + mixed precision"""
    model.eval()
...
def main():
    parser = argparse.ArgumentParser(description="GPU-optimized GraphSAGE")
    ...
    use_amp = training_cfg.get("use_amp", True)  # Enable mixed precision by default
    
    print(f"\n{'='*70}")
    print(f"GPU Mini-Batch Training")
    print(f"{'='*70}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_metrics, _ = evaluate_split_minibatch(
            model, x, edge_index, edge_attr, y_edge, val_idx,
    ...
    train_metrics, train_probs = evaluate_split_minibatch(
        model, x, edge_index, edge_attr, y_edge, train_idx,
        eval_batch_size, device, eval_cfg, use_amp
    )
    ...
    test_metrics, test_probs = evaluate_split_minibatch(
        model, x, edge_index, edge_attr, y_edge, test_idx,
        eval_batch_size, device, eval_cfg, use_amp
    )
